...
def load_command_table(self, _):
    ...
    configstore_keyvalue_util = CliCommandType(
        operations_tmpl='azext_appconfig.keyvalue#{}',
    ...
    configstore_snapshot_util = CliCommandType(
        operations_tmpl='azext_appconfig.snapshot#{}',
    ...
    def get_custom_sdk(custom_module, client_factory, table_transformer):
        ...
        return CliCommandType(
            operations_tmpl='azext_appconfig.{}#'.format(custom_module) + '{}',
            client_factory=client_factory,
            table_transformer=table_transformer
        )

    # Management Plane Commands
    with self.command_group('appconfigdev', configstore_custom_util) as g:
    ...
    with self.command_group('appconfigdev replica', configstore_replica_util, is_preview=True) as g:
    ...
    with self.command_group('appconfigdev credential', configstore_credential_util) as g:
        g.command('list', 'list_credential')
        g.command('regenerate', 'regenerate_credential')

    with self.command_group('appconfigdev identity', configstore_identity_util) as g:
    ...
    with self.command_group('appconfigdev revision', configstore_keyvalue_util) as g:
        g.command('list', 'list_revision')

    # Data Plane Commands
    with self.command_group('appconfigdev kv', configstore_keyvalue_util) as g:
    ...
    with self.command_group('appconfigdev feature',
    ...
    with self.command_group('appconfigdev feature filter',
                            custom_command_type=get_custom_sdk('feature',
                                                               cf_configstore_operations,
    ...
    # Snapshot Commands
    with self.command_group('appconfigdev snapshot', configstore_snapshot_util, is_preview=True) as g:
